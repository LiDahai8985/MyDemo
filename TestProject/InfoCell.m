...
- (void)setup {
    ...
    self.button = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, screenWidth, 100)];
    [self.button addTarget:self action:@selector(buttonEvent) forControlEvents:UIControlEventTouchUpInside];
    [self addSubview:self.button];
    
    self.lineView = [[UIView alloc] initWithFrame:CGRectMake(0, 49.5, screenWidth, 0.5f)];
...
- (void)loadData:(id)data {
    ...
    if (self.model.expend == YES) {
        ...
        self.lineView.frame = CGRectMake(0, 99.5f, [UIScreen mainScreen].bounds.size.width, 0.5f);
        self.infoLabel.frame = CGRectMake(30, 0, 100, 50);
        
    } else {
        
        self.lineView.frame = CGRectMake(0, 49.5, [UIScreen mainScreen].bounds.size.width, 0.5f);
